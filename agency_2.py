...
class DDPGAgent:
    ...
    def policy(self, state):
        ...
        actions = [tf.squeeze(action).numpy() for action in actions]
        if np.random.rand() < self.exploration_rate:
            connect_output = np.random.rand(self.num_users)
            base_station_output = np.random.rand(self.num_users, self.num_stations)
            bandwidth_output = np.random.rand(self.num_users, self.num_stations) * self.bandwidth_capacity
            computing_power_output = np.random.rand(self.num_stations, self.num_tasks) * self.computing_capacity[:, np.newaxis]
            actions = [connect_output, base_station_output, bandwidth_output, computing_power_output]
        return actions
    ...
    def replay(self):
        ...
        for state, action, reward, next_state, done in batch:
            ...
            next_conn_action, next_bs_action, next_bw_action, next_cp_action = self.target_actor(next_state, training=True)

            target_q = reward + self.gamma * self.target_critic(
                [next_state, next_conn_action, next_bs_action, next_bw_action, next_cp_action], training=True) * (1 - done)
            with tf.GradientTape() as tape:
                q_value = self.critic([state, conn_action, bs_action, bw_action, cp_action], training=True)
            ...
            with tf.GradientTape() as tape:
                ...
                actor_loss = -self.critic([state] + actions, training=True)
