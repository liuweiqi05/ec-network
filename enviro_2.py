...
class TaskEnvironment(gym.Env):
    def __init__(self, num_ue, num_bs, num_tsk, bw_cap, cp_cap,
        ...
        self.state_dim = (num_ue * 2) + (num_bs * 2) + (num_ue * num_tsk) + num_tsk + (num_ue * num_tsk) + (num_bs * 2) + num_ue + (num_ue * num_bs)
    ...
    def reset(self):
        ...
        task_sizes = np.random.uniform(low=0.4, high=0.8, size=self.num_tasks)

        bit_sizes = np.random.uniform(low=self.task_size_range[0], high=self.task_size_range[1], size=(self.num_users * self.num_tasks))

        users_init_time = 9 * np.ones((self.num_users * self.num_tasks))

        users_init_con, allo_bw = -1 * np.ones(self.num_users), np.zeros((self.num_users * self.num_stations))

        allo_cp = np.zeros(self.num_stations)

        self.state = np.concatenate((users_locations, self.predefined_bs_locations.flatten(), bit_sizes,
                                     task_sizes, users_init_time, users_init_con, allo_bw, self.bandwidth_capacity, self.computing_capacity))
        return self.state
    ...
    def step(self, action):
        ...
        base_station_probs = action[self.num_users:self.num_users + self.num_users * self.num_stations].reshape(
            (self.num_users, self.num_stations))
        bw_allocation = action[
        ...
        # print(connection, bandwidths)
        x_lo = self.state[:2 * self.num_users].reshape((self.num_users, 2))
        bs_lo = self.state[self.num_users * 2:self.num_users * 2 + self.num_stations * 2].reshape(
            (self.num_stations, 2))

        task_sizes = self.state[
                     self.num_users * 2 + self.num_stations * 2:self.num_users * 2 + self.num_stations * 2 + self.num_users * self.num_tasks].reshape(
            (self.num_users, self.num_tasks))
        bit_size = self.state[
                   self.num_users * 2 + self.num_stations * 2 + self.num_users * self.num_tasks:self.num_users * 2 + self.num_stations * 2 + self.num_users * self.num_tasks + self.num_tasks]
        ...
        for i in range(self.num_users):
            if connection[i] == -1:
                ...
                t_w, t_c = task_sizes[i] / R, tsk_S_bs[connection[i]] * bit_size * 1000 * 100 / (
                        cp_allocation[connection[i]] * 15e9)
                tsk_t[i] = t_w + t_c
        t_pos = np.sum(tsk_t, axis=1)
        ...
        task_sizes = self.state[
                     self.num_users * 2 + self.num_stations * 2:self.num_users * 2 + self.num_stations * 2 + self.num_users * self.num_tasks]
        bit_size = self.state[
                   self.num_users * 2 + self.num_stations * 2 + self.num_users * self.num_tasks:self.num_users * 2 + self.num_stations * 2 + self.num_users * self.num_tasks + self.num_tasks]
        tsk_t = tsk_t.flatten()
        bandwidths_allo = bandwidths.flatten()
        self.state = np.concatenate((users_locations, base_stations_locations, bit_size, task_sizes, tsk_t, connection,
                                     bandwidths_allo, self.bandwidth_capacity, self.computing_capacity))
        reward = -np.sum(t_pos) * 100 / self.num_tasks
        ...
        return t_pos, self.state, reward, done, {}
